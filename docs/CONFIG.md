...
| `wrap_input`              | boolean | Soft-wrap long lines in the prompt editor on word boundaries (display-only — no line feeds are inserted into typed text or pastes). Default: `false`, which keeps a long line on one row and scrolls it horizontally so the caret stays visible. |
...
| `mouse_capture`           | boolean | When `true` (default), the TUI captures mouse events so zerostack can handle scrolling, click-to-place-cursor, and drag-to-select/copy. When `false`, mouse selection/copy/paste is handled by your terminal emulator instead, but in-app mouse features — including mouse-wheel scrolling — are disabled. Use `PgUp`/`PgDn`, `Ctrl+Up`/`Ctrl+Down`, or `Home`/`End` (with `multiline_prompt`: `Ctrl+Home`/`Ctrl+End`) to scroll when mouse capture is off. |
| `clipboard_selection`     | string  | Which selection copy operations (`drag-select` release, `y`) write to: `clipboard` (system clipboard, pasted with `Ctrl+V`/`Ctrl+Shift+V`) or `primary` (X11 PRIMARY selection, pasted with the middle mouse button). Copying tries `wl-copy`/`xclip`/`xsel`/`pbcopy`/`clip.exe` and falls back to the OSC 52 terminal escape. Default: `clipboard`. |
| `multiline_prompt`        | boolean | Multiline prompt mode: swap the prompt-editor roles of `Enter` and `Ctrl+J`. Default `false`: `Enter` sends the message, `Ctrl+J` inserts a newline. When `true`: `Enter` inserts a newline, `Ctrl+J` sends — for terminals that cannot encode `Shift`/`Alt+Enter` (xterm, the Linux console, GNU screen, mosh), where `Ctrl+J` (byte `0x0A`) is the only portable newline key. `Shift`/`Alt+Enter` always insert a newline; pasted newlines are unaffected either way. A buffer that starts with a slash command (after leading whitespace) is an exception: `Enter` always submits it, so a command completed from the picker runs without a chord. It also remaps the transcript jumps: `Home`/`End` move the prompt cursor to the start / end of the buffer (first line first char, last line past the last char), and the transcript jumps move to `Ctrl+Home` / `Ctrl+End`. |
| `quit_armed`              | boolean | When `true`, quitting the TUI with `Ctrl-C` or `Ctrl-D` requires two presses: the first idle press arms a pending quit and prints a hint, any other key cancels it, and a second consecutive press of either key exits. Default `false` (a single press quits). `Ctrl-D` only counts as a quit while the prompt is empty; with text present it deletes the character under the cursor. |
| `esc_alt_compat`          | boolean | Fold `Esc`-prefixed `Alt` sequences for terminals that cannot send `Alt+<key>` as one modified key press (xterm without `mode-2017`, the Linux virtual console, GNU screen, mosh, slow SSH links). When `true`, a lone `Esc` is held for 250 ms: a following character or `Enter` becomes `Alt` + that key (so `Alt+B`/`Alt+F`/`Alt+D`/`Alt+Y`/`Alt+M`/`Alt+P` work), a second `Esc` collapses into a single `Esc`, and any other key releases the held `Esc` first. Default `false` (keys pass through exactly as the terminal sends them). |
...
| `pager`                   | string  | Pager command for the read-only transcript view (`Ctrl+T`, `/transcript`). A full command line, so arguments are allowed (e.g. `view -`, `less -R`). Overrides `$PAGER`; when unset, `$PAGER` is used, then `less`. The transcript is piped to the pager's stdin. |
