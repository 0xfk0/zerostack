...
| `double_ctrl_d`           | boolean | When `true`, quitting the TUI with `Ctrl-D` requires two presses: the first arms a pending quit and prints a hint, any other key cancels it, and a second consecutive `Ctrl-D` exits. `Ctrl-C` is unaffected and always quits immediately when idle. Default `false` (a single `Ctrl-D` quits). |
