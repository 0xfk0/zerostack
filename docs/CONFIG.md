...
| `quit_armed`              | boolean | When `true`, quitting the TUI with `Ctrl-C` or `Ctrl-D` requires two presses: the first idle press arms a pending quit and prints a hint, any other key cancels it, and a second consecutive press of either key exits. Default `false` (a single press quits). |
